...
public class CannonView extends SurfaceView
    ...
    private Activity activity; // to display Game Over dialoge in GUI thread
    private boolean dialogIsDisplayed = false;

    // constants for game play 
    private static int LEVEL_COUNT = 1; // The current level of the player
    public static int TARGET_PIECES = LEVEL_COUNT; // sections in the target
    private static int MAX_LEVEL = 9; // The max level is till 9
    public static final int MISS_PENALTY = 2; // seconds deducted on a miss
    public static final int HIT_REWARD = 10; // seconds added on a hit
    private int playerScore = 0; // Score of the player
...
// variables for the cannon and cannon ball
    private Point cannonball; // cannon ball image's upper-left corner
    private int cannonballVelocityX; // cannonball's x velocity
    private int cannonballVelocityY; // cannonball's y velocity
    private boolean cannonballOnScreen; // whether cannon ball on the screen
    ...
    private Point barrelEnd; // the end point of the cannon's barrel
...
// Paint variables used when drawing each item on the screen
    ...
    private Paint cannonballPaint; // Paint used to draw the cannon ball
...
// initialize Lines and Point representing game items
        ...
        cannonball = new Point(); // create the cannon ball as a Point
...
// construct Paints for drawing text, cannon ball, cannon,
...
// such as when it's first added to the View hierarchy
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        cannonballRadius = w / 36; // cannon ball radius 1/36 screen width
        cannonballSpeed = w * 3 / 2; // cannon ball speed multiplier
...
// reset all the screen elements and start a new game
    public void newGame() {
        hitStates = new boolean[TARGET_PIECES + 1];
...
// set every element of hitStates to false--restores target pieces
        ...
        timeLeft = 10; // start the count down at 10 seconds
        cannonballOnScreen = false; // the cannon ball is not on the screen
...
// called repeatedly by the Cannon Thread to update game elements
...
// check for collision with blocker
            if (cannonball.x + cannonballRadius > blockerDistance
                ...
                cannonballVelocityX *= -1; // reverse cannon ball's direction
                timeLeft -= MISS_PENALTY; // penalize the user
               if ( playerScore - 15 * LEVEL_COUNT < 0 ){
            	   playerScore = 0;
               }
               else
            	   playerScore -= 15 * LEVEL_COUNT;
...
// play blocker sound
                ...
                cannonballOnScreen = false; // remove cannon ball from screen
            } // check for collisions with top and bottom walls
            else if (cannonball.y + cannonballRadius > screenHeight
                    || cannonball.y - cannonballRadius < 0) {
                cannonballOnScreen = false; // remove cannon ball from screen
            } // check for cannon ball collision with target
...
// check if the piece hasn't been hit yet
                if ((section >= 0 && section < TARGET_PIECES) && !hitStates[section]) {
                    ...
                    cannonballOnScreen = false; // remove cannon ball
                    timeLeft += HIT_REWARD; // add reward to remaining time
                    playerScore += 10 * LEVEL_COUNT;
...
// if all pieces have been hit
                    if (++targetPiecesHit == TARGET_PIECES) {
                        ...
                        if (LEVEL_COUNT == MAX_LEVEL) {
                            LEVEL_COUNT = 1;
                            TARGET_PIECES = LEVEL_COUNT;
                            insertHighscores(playerScore, R.string.win); // show winning dialog
                        } else {
                            LEVEL_COUNT += 1;
                            TARGET_PIECES = LEVEL_COUNT;
                            insertHighscores(playerScore, R.string.nextLevel);
                        }
                    }
                }
            }
        ...
        double moveSpeed = interval * LEVEL_COUNT / 2.21;

// update the blocker's position
        double blockerUpdate = moveSpeed * blockerVelocity;
        blocker.start.y += blockerUpdate;
        blocker.end.y += blockerUpdate;

// update the target's position
 double targetUpdate = moveSpeed * targetVelocity;
...
// fires a cannon ball
    public void fireCannonball(MotionEvent event) {
        if (cannonballOnScreen) // if a cannon ball is already on the screen
        {
            return; // do nothing
        }
        double angle = alignCannon(event); // get the cannon barrel's angle

// move the cannon ball to be inside the cannon
...
// get the y component of the total velocity
        ...
        cannonballOnScreen = true; // the cannon ball is on the screen
...
// display time remaining
        canvas.drawText(
                getResources().getString(R.string.time_remaining_format, timeLeft, playerScore), 10, 50, textPaint);
        canvas.drawText(
                getResources().getString(R.string.Level, TARGET_PIECES), 10, 80, textPaint);

// if a cannon ball is currently on the screen, draw it
    ...
    private void showHighscoresDialog(final int messageId) {
        final DialogFragment highScoresDialog = new DialogFragment() {
            ...
            public Dialog onCreateDialog(Bundle bundle) {
                ...
                for (int i = 1; i < 6; i++) {
                    highScoresString += "High Score " + i + ":\t " + highscores[i - 1] + "\n";
                }
...
// create an AlertDialog and return it
            @Override
            public Dialog onCreateDialog(Bundle bundle) {
// create dialog displaying String resource
                AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
...
// user touched the screen or dragged along the screen
        if (action == MotionEvent.ACTION_DOWN
                || action == MotionEvent.ACTION_MOVE) {
            fireCannonball(e); // fire the cannon ball toward the touch point
        }
